...
пользователя для phpMyAdmin и для mqtt брокера. Это удобно сделать через оснастку webmin. 
...
любое, которое будет потом использоваться в настройках плагина, например  mqttUsersDB. После создания базы, необходимо
...
Наконец ранее созданному пользователю для mqtt брокера надо назначить права к этой базе на SELECT,INSERT,UPDATE,DELETE
